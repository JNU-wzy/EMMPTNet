...
class BiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        self.fc = nn.Linear(hidden_size * 2, output_size)  # 因为是双向LSTM，所以需要乘以2
    ...
    def forward(self, x):
        ...
        out = self.fc(out[:, -1, :])  # 只取最后一个时间步的输出 (1,256)
        return out
...
#
#         # out = self.fc(avg_out)  # (batch_size, output_size)
#         return avg_out, attn_weights
...
#         self.bilstm_selfatt = BiLSTMWithAttention(input_dim=14, hidden_dim=128, embed_dim=256, num_heads=4, num_layers=2, dropout=0.1)
#         self.in_feats_seq = 128  # 输入维度
#         self.h_feats_seq = 128
#         self.o_feats = 256
#         self.conv1_seq = GraphConv(self.in_feats_seq, self.h_feats_seq, norm='none', allow_zero_in_degree=True)
