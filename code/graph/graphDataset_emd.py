...
class TrainDataset(DGLDataset):
    ...
    def save(self):
        ...
        save_graphs(self.save_dir + ".bin", self.graphs)
    ...
    def load(self):
        ...
        self.graphs = load_graphs(self.save_dir + ".bin")
    ...
    def vectorize(self, method="ERNIE-RNA", feaSize=params.d, window=5, sg=1,
        ...
        if method == "RNA_fm":
            with open(f'Node_feature/RNA_fm_k5_d640.pkl', 'rb') as f:
                self.vector['embedding'] = pickle.load(f)
        if method == "ERNIE-RNA":
            with open(f'Node_feature/ERNIE-RNA_k5_d768.pkl', 'rb') as f:
                self.vector['embedding'] = pickle.load(f)
        ...
        if method == 'word2vec':
            ...
            model = FastText(sentences=self.k_RNA, vector_size=feaSize, window=window, sg=sg, workers=workers)
            self.vector['embedding'] = np.array([model.wv[kmers] for kmers in self.id2kmers])

        elif method == 'onehot':
            enc = OneHotEncoder(categories='auto', sparse=False)
            enc.fit([[i] for i in self.kmers2id.values()])  # Fit encoder with k-mer IDs

            feaSize = len(self.kmers2id)  # Number of unique k-mers

            kmers_onehot = np.zeros((1428, feaSize), dtype=np.float32)
            bs = 50000
            print('Getting the one-hot encoded kmers vector...')
            for i, t in enumerate(self.idSeq):
                for j in range((len(t) + bs - 1) // bs):
                    # Transform k-mer IDs into one-hot vectors
                    transformed = enc.transform(np.array(t[j * bs:(j + 1) * bs]).reshape(-1, 1))
                    # Accumulate the one-hot vectors
                    for k in range(len(transformed)):
                        kmers_onehot[i, :] += transformed[k, :]

            # Store the one-hot encoded vectors in self.vector['embedding']
            self.vector['embedding'] = kmers_onehot
        # Save embedding or encoder
        with open(f'Node_feature/{method}_k{self.kmers}_d{feaSize}.pkl', 'wb') as f:
            pickle.dump(self.vector['embedding'], f, protocol=4)


params = config()
dataset = TrainDataset(raw_dir=r"D:\Project\Pytorch_project\binding affinity\features\ID_smiles_seq.xlsx",
                       save_dir=rf'D:\Project\Pytorch_project\binding affinity\code\graph\dglgraph\ERNIE-RNA_k{params.k}_d{params.d}_smiles')
